...
class talk_to_svr : public boost::enable_shared_from_this<talk_to_svr>
{
	...
	talk_to_svr(const std::string& username, boost::shared_ptr<io_service> service)
		: sock_(*service), username_(username), timer_(*service) {}
...
public:
	...
	typedef boost::shared_ptr<talk_to_svr> tcp_client_ptr;

	static tcp_client_ptr start(ip::tcp::endpoint ep, const std::string& username, boost::shared_ptr<io_service> service)
	{
		tcp_client_ptr new_(new talk_to_svr(username, service));
		new_->start(ep);
		return new_;
	}
	void stop()
